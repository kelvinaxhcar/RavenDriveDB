...
class SimulacaoRavenDbNoGoogleDrive {
  ...
  public SimulacaoRavenDbNoGoogleDrive(string credencialPath,
                                       string idDaColecaoPasta) {
  ...
  public async Task CriarDocumentoAsync<T>(T objeto)
      where T : class {
    try {
      string nomeDoDocumentoTemp = $"{typeof(T).Name}-{Guid.NewGuid()}-temp";
      string conteudoDoDocumento =
          JsonConvert.SerializeObject(objeto, Formatting.Indented);
      ...
      var uploadRequest =
          _service.Files.Create(fileMetadata, mediaContent, MimeTypeJson);
      ...
      await AtualizarNomeDocumentoAsync(uploadRequest.ResponseBody.Id,
                                        novoNome);

      Console.WriteLine(
          $"Documento '{novoNome}' criado e atualizado com sucesso! ID: {uploadRequest.ResponseBody.Id}");
    } catch (Exception ex) {
  ...
  private async Task AtualizarNomeDocumentoAsync(string documentoId,
                                                 string novoNome) {
    var updateMetadata = new Google.Apis.Drive.v3.Data.File { Name = novoNome };
  ...
  public async Task<T> LerDocumentoAsync<T>(string documentoId) {
    try {
      ...
      Console.WriteLine(
          $"Documento com ID {documentoId} lido com sucesso e deserializado.");
      return objeto;
    } catch (Exception ex) {
  ...
  public async Task<List<T>> ConsultarDocumentosAsync<T>(Func<T, bool> criterio)
      where T : class {
    ...
    try {
      var request = _service.Files.List();
      request.Q =
          $"'{_idDaColecaoPasta}' in parents and mimeType='{MimeTypeJson}'";
      ...
      if (result.Files != null && result.Files.Any()) {
        var tarefas =
            result.Files.Select(file => LerDocumentoAsync<T>(file.Id));
  ...
  public async Task AtualizarDocumentoAsync<T>(string documentoId, T novoObjeto)
      where T : class {
    try {
      string conteudoAtualizado =
          JsonConvert.SerializeObject(novoObjeto, Formatting.Indented);
      using var mediaContent = CriarMemoryStream(conteudoAtualizado);

      var updateMetadata = new Google.Apis.Drive.v3.Data.File();
      var updateRequest = _service.Files.Update(updateMetadata, documentoId,
                                                mediaContent, MimeTypeJson);
      updateRequest.Fields = "id, name";
      ...
      Console.WriteLine(
          $"Documento com ID {documentoId} atualizado com sucesso!");
    } catch (Exception ex) {
...
class Program {
  static async Task Main(string[] args) {
    ...
    var ravenDbSimulado =
        new SimulacaoRavenDbNoGoogleDrive(credencialPath, idDaColecaoPasta);
    ...
    var produtosFiltrados =
        await ravenDbSimulado.ConsultarDocumentosAsync<Produto>(
            p => p.Nome == "Notebook");
    ...
    var produtoAtualizado = new Produto {
      Id = "1HZatRt7Wn4zJHEhan87f8UrOXdhYov71", // ID do documento a ser
                                                // atualizado
      Nome = "Notebook Atualizado ss", Preco = 2999.99M
    };

    await ravenDbSimulado.AtualizarDocumentoAsync(produtoAtualizado.Id,
                                                  produtoAtualizado);

    var produtoLido = await ravenDbSimulado.LerDocumentoAsync<Produto>(
        "1HZatRt7Wn4zJHEhan87f8UrOXdhYov71");
    Console.WriteLine(
        $"Produto lido: {produtoLido.Nome} - {produtoLido.Preco:C}");
  }
}
